...
class RegisterUser(APIView):
    def post(self, request):
        ...
        if added == 0:
            return Response({'error': 'No faces detected. Is HF Space running?'}, status=400)

        user.save()
        return Response({'message': f"Registered '{name}' with {added} photo(s).", 'failed': failed, 'photo_count': user.photo_count, 'user_id': user.id}, status=201)
        ...
        for key, f in request.FILES.items():
            if key.startswith('image'):
                ...
                if img:
                    emb = get_embedding(img)
                    if emb is not None:
                        user.add_embedding(emb); added += 1
        if not added:
            return Response({'error': 'No valid faces.'}, status=400)
        user.save()
...
class ScanFrame(APIView):
    def post(self, request):
        ...
        return Response({'detections': detections, 'face_count': len(detections), 'known_count': sum(1 for d in detections if d['name'] not in ('Unknown', 'SPOOF'))})
...
class AttendanceLogView(APIView):
    def get(self, request):
        ...
        if n := request.query_params.get('name'): qs = qs.filter(user__name__icontains=n)
        if e := request.query_params.get('event'):
            if e in ('entry', 'exit'): qs = qs.filter(event_type=e)
        if d := request.query_params.get('date'):
            try: qs = qs.filter(timestamp__date=datetime.strptime(d, '%Y-%m-%d').date())
...
class AttendanceSessionView(APIView):
    def get(self, request):
        ...
        if d := request.query_params.get('date'):
            try: qs = qs.filter(date=datetime.strptime(d, '%Y-%m-%d').date())
            except ValueError: pass
...
class AnalyticsView(APIView):
    def get(self, request):
        ...
        for i in range(6, -1, -1):
            ...
            count = AttendanceLog.objects.filter(timestamp__date=d, event_type='entry').values('user').distinct().count()
        ...
        for log in today_entries: hourly[log.timestamp.hour] += 1
        hourly_data = [{'hour': h, 'label': f'{h:02d}:00', 'count': hourly.get(h, 0)} for h in range(6, 22)]
        peak = max(hourly_data, key=lambda x: x['count'], default=None)
        peak_hour = peak['label'] if peak and peak['count'] > 0 else 'N/A'

        top_attendees = list(AttendanceLog.objects.filter(event_type='entry').values('user__name', 'user__student_id').annotate(total=Count('id')).order_by('-total')[:5])
        avg_d = AttendanceSession.objects.filter(date=today, duration_minutes__isnull=False).aggregate(avg=Avg('duration_minutes'))['avg']
        present_users = list(UserProfile.objects.filter(is_present=True).values('id', 'name', 'student_id', 'department', 'last_seen'))

        return Response({
            'total_users': total, 'present_now': present_now,
            'present_today': present_today, 'attendance_rate': rate,
            'late_today': late, 'week_data': week_data,
            'week_avg': week_avg, 'week_total': week_total,
            'hourly_data': hourly_data, 'peak_hour': peak_hour,
...
class SemanticQueryView(APIView):
    ...
    Body: { "query": "When did Yash arrive?", "mode": "groq" | "ollama" }
    """
    def post(self, request):
        ...
        if mode == 'ollama':
            ...
            answer = query_ollama(prompt, host=ollama_host, model=ollama_model, system=_SEMANTIC_SYSTEM)
        else:
            groq_key   = getattr(settings, 'GROQ_API_KEY', '')
            groq_model = getattr(settings, 'GROQ_MODEL',   'llama-3.1-8b-instant')
            answer = query_groq(prompt, api_key=groq_key, model=groq_model, system=_SEMANTIC_SYSTEM)

        return Response({'answer': answer, 'data': records, 'context_lines': len(context.splitlines())})
...
class AIInsightView(APIView):
    def post(self, request):
        ...
        total  = UserProfile.objects.count()
        present = AttendanceLog.objects.filter(timestamp__date=today, event_type='entry').values('user').distinct().count()
        stats = {
            'total_users': total, 'present_today': present,
            'attendance_rate_today': round(present / total * 100, 1) if total else 0,
            'late_today': AttendanceLog.objects.filter(timestamp__date=today, event_type='entry', timestamp__hour__gte=9).values('user').distinct().count(),
            'week_total': AttendanceLog.objects.filter(timestamp__date__gte=today - timedelta(days=6), event_type='entry').count(),
            'week_avg': 0, 'top_attendee': 'N/A', 'peak_hour': 'unknown',
        }
        top = AttendanceLog.objects.filter(event_type='entry').values('user__name').annotate(t=Count('id')).order_by('-t').first()
        if top: stats['top_attendee'] = top['user__name']
        prompt = custom if custom else build_analytics_prompt(stats)
        if mode == 'ollama':
            result = query_ollama(prompt, host=getattr(settings, 'OLLAMA_HOST', 'http://localhost:11434'), model=getattr(settings, 'OLLAMA_MODEL', 'llama3.2:1b'))
        else:
            result = query_groq(prompt, api_key=getattr(settings, 'GROQ_API_KEY', ''), model=getattr(settings, 'GROQ_MODEL', 'llama-3.1-8b-instant'))
...
class ExportCSV(APIView):
    def get(self, request):
        ...
        try: d = datetime.strptime(d_str, '%Y-%m-%d').date()
        ...
        for log in logs:
            w.writerow([log.user.name, log.user.student_id, log.user.department, log.event_type, log.timestamp.strftime('%H:%M:%S'), round(log.confidence * 100, 1)])
...
class HealthCheck(APIView):
    def get(self, request):
        return Response({'status': 'ok', 'users': UserProfile.objects.count(), 'hf_space': getattr(settings, 'HF_SPACE_URL', 'not set')})

def root(request):
    from django.http import JsonResponse
    return JsonResponse({'service': 'Presenza Backend', 'status': 'running', 'api': '/api/health/'})
